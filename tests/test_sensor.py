...
def test_sleep_record_sensor_keyerror_branch():
    desc = SensorEntityDescription(key="not_a_real_key")
    coordinator = DummyCoordinator({"sleep_records": [{}]})
...
def test_sleep_record_sensor_no_sleep_records_branch():
    desc = SensorEntityDescription(key="any_key")
...
def test_device_sensor_keyerror_branch():
    desc = SensorEntityDescription(key="not_a_real_key")
...
def test_device_sensor_no_device_branch():
    desc = SensorEntityDescription(key="any_key")
    coordinator = DummyCoordinator({})
    sensor = MyAirDeviceSensor("Test", desc, coordinator)
    with patch.object(sensor, "async_write_ha_state", return_value=None):
...
def test_sleep_record_sensor_handle_coordinator_update(
    ...
    def test_myair_device_sensor_handle_coordinator_update(
        ...
        async def test_async_setup_entry_adds_entities_and_registers_service(monkeypatch):
            ...
            config_entry.data = {"user_name": "test.user@email.com"}
            ...
            assert (
                kwargs == {"update_before_add": False}
                or kwargs == {"update_before_add": False}
                or kwargs == {"update_before_add": False}
            )  # Defensive for HA versions
