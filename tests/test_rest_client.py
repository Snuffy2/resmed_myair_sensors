...
@pytest.mark.asyncio
async def test_is_email_verified_true(config_na, session):
    """Test is_email_verified returns True."""
    client = RESTClient(config_na, session)
    ...
    with patch.object(RESTClient, "_resmed_response_error_check", new=AsyncMock()):
        assert await client.is_email_verified() is True


@pytest.mark.asyncio
async def test_is_email_verified_false(config_na, session):
    """Test is_email_verified returns False."""
    client = RESTClient(config_na, session)
    client._access_token = "token"
    session.get.return_value.__aenter__.return_value.json = AsyncMock(
    ...
    session.get.return_value.__aenter__.return_value.status = 200
    with patch.object(RESTClient, "_resmed_response_error_check", new=AsyncMock()):
        assert await client.is_email_verified() is False


@pytest.mark.asyncio
async def test_extract_and_update_cookies(config_na, session):
    """Test _extract_and_update_cookies updates cookies."""
    client = RESTClient(config_na, session)
    await client._extract_and_update_cookies(["DT=abc; Path=/", "sid=xyz; Path=/"])
    assert client._cookie_dt == "abc"
    assert client._cookie_sid == "xyz"


@pytest.mark.asyncio
async def test_extract_and_update_cookies_no_change(config_na, session):
    """Test _extract_and_update_cookies does not update if cookies are the same."""
    client = RESTClient(config_na, session)
    client._cookie_dt = "abc"
    client._cookie_sid = "xyz"
    await client._extract_and_update_cookies(["DT=abc; Path=/", "sid=xyz; Path=/"])
    assert client._cookie_dt == "abc"
    assert client._cookie_sid == "xyz"
...
@pytest.mark.asyncio
async def test_is_access_token_active_true(config_na, session):
    """Test _is_access_token_active returns True."""
    client = RESTClient(config_na, session)
    client._access_token = "token"
    ...
    with patch.object(RESTClient, "_resmed_response_error_check", new=AsyncMock()):
        assert await client._is_access_token_active() is True


@pytest.mark.asyncio
async def test_is_access_token_active_false(config_na, session):
    """Test _is_access_token_active returns False."""
    client = RESTClient(config_na, session)
    client._access_token = "token"
    session.post.return_value.__aenter__.return_value.json = AsyncMock(
    ...
    session.post.return_value.__aenter__.return_value.status = 200
    with patch.object(RESTClient, "_resmed_response_error_check", new=AsyncMock()):
        assert await client._is_access_token_active() is False
...
async def test_get_sleep_records_missing_keys(config_na, session):
    ...
    with patch.object(client, "_gql_query", AsyncMock(return_value=mock_response)):
        with pytest.raises(ParsingError, match="Error getting Patient Sleep Records"):
...
async def test_get_sleep_records_not_a_list(config_na, session):
    ...
    with patch.object(client, "_gql_query", AsyncMock(return_value=mock_response)):
        with pytest.raises(ParsingError, match="Returned records is not a list"):
            await client.get_sleep_records()
...
async def test_get_user_device_data_missing_keys(config_na, session):
    ...
    mock_response = {"data": {"getPatientWrapper": {}}}
    with patch.object(client, "_gql_query", AsyncMock(return_value=mock_response)):
        with pytest.raises(ParsingError, match="Error getting User Device Data"):
...
async def test_get_user_device_data_not_a_dict(config_na, session):
    ...
    with patch.object(client, "_gql_query", AsyncMock(return_value=mock_response)):
        with pytest.raises(ParsingError, match="Returned data is not a dict"):
            await client.get_user_device_data()


@pytest.mark.asyncio
async def test_get_initial_dt_success(config_na, session):
    client = RESTClient(config_na, session)
    mock_headers = MagicMock()
    mock_headers.getall = MagicMock(return_value=["DT=token; Path=/;"])
    ...
    with (
        ...
        mock_extract.assert_called_once_with(["DT=token; Path=/;"])


@pytest.mark.asyncio
async def test_get_initial_dt_no_cookies(config_na, session):
    client = RESTClient(config_na, session)
    mock_headers = MagicMock()
    mock_headers.getall = MagicMock(return_value=[])
    mock_response = MagicMock()
    mock_response.headers = mock_headers
